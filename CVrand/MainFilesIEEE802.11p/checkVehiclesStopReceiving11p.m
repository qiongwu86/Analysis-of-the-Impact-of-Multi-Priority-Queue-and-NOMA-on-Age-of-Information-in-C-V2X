...
if simParams.technology == 4 && simParams.coexMethod~=0 && simParams.coex_slotManagement==2 && simParams.coex_cbrTotVariant==2
    ...
    % lowSensedPower = (rxPowerTotNow_PartA(stationManagement.activeIDsLTE) + interferenceFromLTEnodesPerSubframe ) < simParams.coex_powerStopSensing11p;
    lowSensedPower = (rxPowerTotNow_PartA(stationManagement.activeIDsLTE) + interferenceFromLTEnodesPerSubframe ) < phyParams.PrxSensWhenSynch;
    %%
    ifStopDetecting11p = logical(sinrManagement.coex_detecting11p(stationManagement.activeIDsLTE)...
        .* lowSensedPower);
    sinrManagement.coex_detecting11p(stationManagement.activeIDsLTE(ifStopDetecting11p)) = false; 
    stationManagement.channelSensedBusyMatrix11p(1,stationManagement.activeIDsLTE(ifStopDetecting11p)) = stationManagement.channelSensedBusyMatrix11p(1,stationManagement.activeIDsLTE(ifStopDetecting11p)) + (timeManagement.timeNow-timeManagement.cbr11p_timeStartBusy(stationManagement.activeIDsLTE(ifStopDetecting11p))');
    timeManagement.cbr11p_timeStartBusy(stationManagement.activeIDsLTE(ifStopDetecting11p)) = -1;
