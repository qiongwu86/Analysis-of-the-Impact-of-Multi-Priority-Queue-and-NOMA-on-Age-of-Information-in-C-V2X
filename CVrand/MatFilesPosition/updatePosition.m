...
if ~isreal(Xmax)%ETSI-Urban
    ...
    Xvehicle = (~direction).*(Xvehicle + v*updateInterval) + direction.*(Xvehicle - v*updateInterval);
    IDvehicleExit = find((~direction).*Xvehicle>Xmax | direction.*Xvehicle<0 );

    Xvehicle(IDvehicleExit)=0;
    if IDvehicleExit>0
%         Xvehicle
        indexNewVehicles = IDvehicleExit;
        Xvehicle(indexNewVehicles) = (~direction(indexNewVehicles)).*mod(Xvehicle(indexNewVehicles) + v(indexNewVehicles)*updateInterval,Xmax) + direction(indexNewVehicles).*mod(Xvehicle(indexNewVehicles) - v(indexNewVehicles)*updateInterval,Xmax);
    else
        indexNewVehicles = [];
    end
...
%     indexNewVehicles = [];
    indexOldVehicles = IDvehicle;
    indexOldVehicles(indexNewVehicles) = [];
    indexOldVehiclesToOld = IDvehicle;
    indexOldVehiclesToOld(indexNewVehicles) = [];
%     IDvehicleExit = [];
