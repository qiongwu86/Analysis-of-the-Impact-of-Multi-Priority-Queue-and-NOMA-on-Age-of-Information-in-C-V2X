...
reverseStr = '';
% fileIDdan = fopen('Rho100-23-100-sin.txt', 'w+');
% fileIDduo = fopen('Rho100-23-100-mul.txt', 'w+');
fileIDdantr = fopen('Rho100-23-100-sin-tr.txt', 'w+');
fileIDduotr = fopen('Rho100-23-100-mul-tr.txt', 'w+');
powdb = 23;   R = 3;
figure
xdata = [];  % 记录历史x数据的数组
ydata = [];  % 记录历史y数据的数组
...
b = round(TC*rand(N,1));
stationManagement.pckBuffer = zeros(N,L,4);  stationManagement.genetime = zeros(N,4*L,4);
for i = 1:length(a)
    stationManagement.pckBuffer(i,1:a(i),3) = b(i) + flip(TC*((1:a(i))-1));  %单队列时CAM信息在前面
end

stationManagement.PHIt = repmat(stationManagement.pckBuffer(:, 1, 1), 1, N);%/*接受端信息年龄*/
...
stationManagement.pckBufferd = zeros(N,4*L,4);
stationManagement.pckBufferd(:,1:10,3) = stationManagement.pckBuffer(:,:,3);
stationManagement.PHItd = stationManagement.PHIt;
for i = 1:N
    % abcd=fliplr(stationManagement.pckBuffer(i,1:a(i),3))
    ai = 1:a(i);
    stationManagement.genetime(i,1:a(i),3) = max(stationManagement.genetime(:)) + ai;%fliplr(stationManagement.pckBuffer(i,1:a(i),3));
end
...
%                     customer(j,totnum(j),gtime)=t;
        ...
        %             -timeManagement.addedToGenerationTime(j);
                    ltd = sum(sum(sum(stationManagement.genetime(j,:,:)>0)) < L*4);  %目前所有类型包的数量应<=40
                    lti = sum(stationManagement.genetime(j,:,H)>0) + ltd;
                    if lti>0
                        stationManagement.genetime(j,lti,H) = max(stationManagement.genetime(:)) + ltd; %stationManagement.genetime(j,lti,H) + ltd*t;
                    end
...
%                     que_totnum(j)=que_totnum(j)+sum(stationManagement.lt(j,H) < L);                     %成功进入队列的包+sum(stationManagement.lt(j,H) < L)
                    ltd = stationManagement.lt(j,H);
                    stationManagement.lt(j,H) = ltd + sum(ltd < L);
                    ltd = sum(sum(sum(stationManagement.genetime(j,:,:)>0)) < L*4);  %目前所有类型包的数量应<=40
                    lti = sum( stationManagement.genetime(j,:,H)>0) + ltd;
                    if lti>0
                        stationManagement.genetime(j,lti,H) = max(stationManagement.genetime(:)) + ltd;%stationManagement.genetime(j,lti,H) + ltd*t;
...
%                     customer(j,totnum(j),gtime)=t;
                    ...
                    ltd = sum(sum(sum(stationManagement.genetime(j,:,:)>0)) < L*4);  %目前所有类型包的数量应<=40
                    lti = sum(stationManagement.genetime(j,:,D)>0) + ltd;
                    if lti>0
                        stationManagement.genetime(j,lti,D) = max(stationManagement.genetime(:)) + ltd;%stationManagement.genetime(j,lti,D) + ltd*t;
                    end
...
%                     que_totnum(j)=que_totnum(j)+sum(stationManagement.lt(j,D) < L);                     %成功进入队列的包+sum(lt(j,D) < L)
                    ltd = stationManagement.lt(j,D);
                    stationManagement.lt(j,D) = ltd + sum(ltd < L);
                    ltd = sum(sum(sum(stationManagement.genetime(j,:,:)>0)) < L*4);  %目前所有类型包的数量应<=40
                    lti = sum(stationManagement.genetime(j,:,D)>0) + ltd;
                    if lti>0
                        stationManagement.genetime(j,lti,D) = max(stationManagement.genetime(:)) + ltd;%stationManagement.genetime(j,lti,D) + ltd*t;
...
%                     customer(j,totnum(j),gtime)=t;
    %                 timeManagement.timeNextPacket(j,C) = t+max(timeManagement.generationInterval(idEvent),timeManagement.dcc_minInterval(idEvent));
        ...
        %             timeManagement.timeLastPacket(j,C) = t-timeManagement.addedToGenerationTime(j);
                    ...
                    ltd = sum(sum(sum(stationManagement.genetime(j,:,:)>0)) < L*4);  %目前所有类型包的数量应<=40
                    lti = sum(stationManagement.genetime(j,:,C)>0) + ltd;
                    if lti>0
                        stationManagement.genetime(j,lti,C) = max(stationManagement.genetime(:)) + ltd;%stationManagement.genetime(j,lti,C) + ltd*t;
                    end
...
%                     customer(j,totnum(j),gtime)=t;
                    ...
                    ltd = sum(sum(sum(stationManagement.genetime(j,:,:)>0)) < L*4);  %目前所有类型包的数量应<=40
                    lti = sum(stationManagement.genetime(j,:,M)>0) + ltd;
                    if lti>0
                        stationManagement.genetime(j,lti,M) = max(stationManagement.genetime(:)) + ltd;%*t
                    end
...
%                     at(j,M)=0;
            ...
            for j = 1:length(transmittingIDs)
                ...
                slice = stationManagement.genetime(id,1,:);
                genetime1 = reshape(slice, [1, 4]);
                if sum(genetime1(:))>0 
                    genetime1(genetime1<=0) = NaN;
                    [~, gidx] = min(genetime1(:));                
                    std(id,gidx)=1;
                end
...
%             cont = ut.*noAboveThreshold;
            ...
            for j = 1:N               %计算接收端aoi
                ...
                std_j = std(j,:);
                isStd1 = any(std_j == 1);
                stationManagement.PHItd(j, :) = stationManagement.PHItd(j, :) + 1;
                stationManagement.PHItd(j, isUt1 & isStd1) = stationManagement.pckBufferd(j, 1, find(std_j == 1, 1)) + stateavg(j,3);
                stationManagement.PHItd(j, j) = 0;
            end
            noAboveThreshold = distanceReal <= phyParams.Raw;
            stationManagement.PHIt = stationManagement.PHIt.*noAboveThreshold;
            stationManagement.PHItd = stationManagement.PHItd.*noAboveThreshold;
...
%             aoi_loct = (sum(sum(stationManagement.pckBuffer(:,:,:), 3), 2) / 40)';
            ...
            ltd = sum(stationManagement.genetime > 0, 2);
            ltd = reshape(ltd, [N, 4]);
            for j = 1:N    %将队首传输以后更新包的位置和队列长度 
                for k = 1:4
                    ...
                    stdjk = std(j,k);
                    ltdvalue = ltd(j,k); 
                    % if stationManagement.pckBufferd(j,1,k)>0 && stationManagement.pckBufferd(j,1,k)==stationManagement.pckBufferd(j,2,k)
                    %     1;
                    % end
                    if stdjk == 0
                        stationManagement.pckBufferd(j,1:ltdvalue,k) = stationManagement.pckBufferd(j,1:ltdvalue,k) + 1;
                        % stationManagement.genetime(j,1:ltdvalue,k) = stationManagement.genetime(j,1:ltdvalue,k);  
                    elseif stdjk == 1
                        stationManagement.pckBufferd(j,1:ltdvalue-1,k) = stationManagement.pckBufferd(j,2:ltdvalue,k) + 1;
                        stationManagement.genetime(j,1:ltdvalue-1,k) = stationManagement.genetime(j,2:ltdvalue,k);
                        if ltdvalue > 0 %该判断不可删除，防止队列为满时第L个位置不被清零
                            stationManagement.pckBufferd(j,ltdvalue,k) = 0;
                            stationManagement.genetime(j,ltdvalue,k) = 0;
                        end
                        % stationManagement.ltd(j,k) = ltvdalue - stdjk;
                    end
                    % if stationManagement.pckBufferd(j,1,k)>0 && stationManagement.pckBufferd(j,1,k)==stationManagement.pckBufferd(j,2,k)
                    %     1;
                    % end
            ...
            %% 测试
            % % if any(ifreselect)
            %     totPHItavg = mean(stationManagement.PHIt(:))/1000; %aoi_conavg 计算 接收端totPHItavg        
            %     totPHItavgd = mean(stationManagement.PHItd(:))/1000;
            %     pow = 0;%mean(energy(idx));
            %     pow(isnan(pow)) = 0;
            %     % locsum = nnz(stationManagement.pckBufferd);
            %     for i = 1:4    
            %         pckBufferi = stationManagement.pckBuffer(:,:,i);
            %         locsum = nnz(pckBufferi);
            %         if locsum > 0
            %             % AoIlocmul(i) = sum(pckBufferi(:))/locsum/1000;   %mean 
            %             AoIlocmul(i) = sum(pckBufferi(:))/40/1000;
            %         else
            %             AoIlocmul(i) = 0;        
            %         end
            %     end
            %     fprintf(fileIDduo, '%f\n', totPHItavg);% fprintf(fileID, '%f\n', AoIloc);
            %     fprintf(fileIDduo, '%f\n', AoIlocmul(1));
            %     fprintf(fileIDduo, '%f\n', AoIlocmul(2));
            %     fprintf(fileIDduo, '%f\n', AoIlocmul(3));
            %     fprintf(fileIDduo, '%f\n', AoIlocmul(4));
            ...
            % 
            %     for i = 1:4    
            %         pckBufferi = stationManagement.pckBufferd(:,:,i);
            %         locsum = nnz(pckBufferi);
            %         if locsum > 0
            %             % AoIlocsin(i) = sum(pckBufferi(:))/locsum/1000;
            %             AoIlocsin(i) = sum(pckBufferi(:))/40/1000;   %sum /locsum    
            %         else
            %             AoIlocsin(i) = 0;        
            %         end
            %     end
            %     fprintf(fileIDdan, '%f\n', totPHItavgd);% fprintf(fileID, '%f\n', AoIloc);
            %     fprintf(fileIDdan, '%f\n', AoIlocsin(1));
            %     fprintf(fileIDdan, '%f\n', AoIlocsin(2));
            %     fprintf(fileIDdan, '%f\n', AoIlocsin(3));
            %     fprintf(fileIDdan, '%f\n', AoIlocsin(4));
            ...
            for i = 1:4    
                ...
                locsum = nnz(pckBufferi);
                if locsum > 0
                    AoIlocsin(i)=sum(pckBufferi(:))/locsum/1000;   %sum /locsum    
                else
                    AoIlocsin(i) = 0;        
                end
            end
            fprintf(fileIDdantr, '%f\n', AoIlocsin(1));
            fprintf(fileIDdantr, '%f\n', AoIlocsin(2));
            fprintf(fileIDdantr, '%f\n', AoIlocsin(3));
            fprintf(fileIDdantr, '%f\n', AoIlocsin(4));
