...
%     activeIDsExit
    ...
    stationManagement.pckBufferd(activeIDsExit,:,:) = zeros(length(activeIDsExit),40,4);
    stationManagement.PHItd(:,activeIDsExit)=0;
    stationManagement.genetime(activeIDsExit,:,:) = zeros(length(activeIDsExit),40,4);
end
...
if ~isempty(stationManagement.activeIDs(indexNewVehicles))
    ...
    for i = 1:Nexit
        activeIDsExitidx = activeIDsExit(i);
        stationManagement.pckBuffer(activeIDsExitidx,1:a(i),3) = b(i) + flip(100*((1:a(i))-1));
        stationManagement.PHIt(activeIDsExitidx,:) = repmat(stationManagement.pckBuffer((activeIDsExitidx), 1, 3), 1, length(stationManagement.PHIt));%/*接受端信息年龄*/
        stationManagement.PHIt(activeIDsExitidx,:) = stationManagement.PHIt(activeIDsExitidx,:) + round(100*rand(1,length(stationManagement.PHIt)));
    
        stationManagement.pckBufferd(activeIDsExitidx,1:10,3) = stationManagement.pckBuffer(activeIDsExitidx,:,3);
        stationManagement.PHItd(activeIDsExitidx,:) = repmat(stationManagement.pckBuffer((activeIDsExitidx), 1, 3), 1, length(stationManagement.PHIt));
        stationManagement.PHItd(activeIDsExitidx,:) = stationManagement.PHItd(activeIDsExitidx,:) + round(100*rand(1,length(stationManagement.PHIt)));
        ai = 1:a(i);
        stationManagement.genetime(activeIDsExitidx,1:a(i),3) = max(stationManagement.genetime(:)) + ai;%fliplr(stationManagement.pckBuffer(activeIDsExitidx,1:a(i),3));
        % disp(activeIDsExit);
        % if activeIDsExit==12
        %     1;
        % end
