...
if staID==-1
    if isfield(stationManagement,'transmittingIDsLTE') && ~isempty(stationManagement.transmittingIDsLTE)
        ...
        for index=1:nTx
            if isThisTX
                fprintf(fid,'%3.6f\t-1\t-1\t-1\t%d\t-1\t-1\t-1',Time,stationManagement.transmittingIDsLTE(index));
            ...
            if alsoRx && ~isThisTX
                ...
                for j = 1:length(indexNeighborsRX)
                    ...
                    sinrThr=phyParams.LOS(index,indexNeighborsRX(j)).*phyParams.sinrThresholdLTE_LOS+...
                            (1-phyParams.LOS(index,indexNeighborsRX(j)) ).*phyParams.sinrThresholdLTE_NLOS;   
                    if sinrManagement.neighborsSINRaverageLTE(index,indexNeighborsRX(j)) < sinrThr
