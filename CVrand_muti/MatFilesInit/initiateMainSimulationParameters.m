...
if simParams.typeOfScenario==2 % Traffic traces
    ...
    elseif simParams.typeOfScenario==3 % ETSI Highway high speed
        % [roadLength]
        % Length of the road to be simulated (m)
        [simParams,varargin] = addNewParam(simParams,'roadLength',500,'Road Length (m)','double',fileCfg,varargin{1});
        if simParams.roadLength<=0
            error('Error: "simParams.roadLength" cannot be <= 0');
        end
        ...
        % [roadWidth]
        % Width of each lane (m)
        [simParams,varargin] = addNewParam(simParams,'roadWidth',2,'Road Width (m)','double',fileCfg,varargin{1});
        if simParams.roadWidth<0
            error('Error: "simParams.roadWidth" cannot be < 0');
        end
        ...
        % [vMean]
        % Mean speed of vehicles (km/h)
        [simParams,varargin] = addNewParam(simParams,'vMean',100,'Mean speed of vehicles (Km/h)','double',fileCfg,varargin{1});
        if simParams.vMean<0
            error('Error: "simParams.vMean" cannot be < 0');
        end
        ...
        % [rho]
        % Density of vehicles (vehicles/km)  rhomax=xmax/v*车速安全距离 initVehiclePositions
        [simParams,varargin] = addNewParam(simParams,'rho',60,'Density of vehicles (vehicles/km)','double',fileCfg,varargin{1});
        if simParams.rho<=0
            error('Error: "simParams.rho" cannot be <= 0');
        end
        
        % [NLanes]
        % Number of lanes per direction  %%%%%%%%%%%%%%%%%每方向车道数
        [simParams,varargin] = addNewParam(simParams,'NLanes',2,'Number of lanes per direction','integer',fileCfg,varargin{1});
        if simParams.NLanes<=0
            error('Error: "simParams.NLanes" cannot be <= 0');
        end
